...
export const Message: React.FC<MessageType> = ({
   ...
   const [isHoverMessageAction, setIsHoverMessageAction] =
      useState<boolean>(false);

   // const messageLayout = participantId === 1 ? 'my-message' : 'other';
   const isMyMessage = participantId === 1 ? true : false;
      ...
      <>
         <div
            className={`message-wrapper ${
               isMyMessage ? 'my-message' : 'other'
            }`}
            onMouseOver={() => setIsHoverMessageAction(true)}
            onMouseOut={() => setIsHoverMessageAction(false)}
         >
            <img
         ...
         >
            <NameModal participant={participant} time={time} />
         </HoverModal>
      </>
   );
};
